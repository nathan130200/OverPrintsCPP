	...
	namespace NodeEditor {
		namespace Detail {
			...
			#define DECLARE_KEY_TESTER(Key)                                                                          \
				...
				{																			                         \
					template<typename T, typename = std::enable_if<has_nested_ ## Key<Key>::value, T>::value>  \													 \
					static int Get()		                                                                         \
					{																		                         \
						return ImGui::GetKeyIndex(ImGuiKey::Key);									                 \
					}																		                         \
																									                 \
					template<typename T, typename = std::enable_if<has_nested_ ## Key::value<Key>, T>::value>	 \
					static int Get()														                         \
					{																		                         \
						return -1;															                         \
					}																		                         \
			...
			{
				return ImGui::GetKeyIndex(ImGuiKey::ImGuiKey_F);
			}

			static inline int GetKeyIndexForD()
			{
				return ImGui::GetKeyIndex(ImGuiKey::ImGuiKey_D);
			}
